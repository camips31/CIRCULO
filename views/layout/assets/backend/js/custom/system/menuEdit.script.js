...
var MenuEdit = (function () {
    ...
    const formSystemMenu = document.getElementById("system-form-menu-edit");
    ...
    const SystemMenuEditSubmitButton = document.getElementById("system-form-menu-edit-submit");
    SystemMenuEditSubmitButton.addEventListener("click", function (e) { // Prevent default button action
        e.preventDefault();
        ...
        if (validatorSystemMenu) {
            validatorSystemMenu.validate().then(function (status) {
                ...
                if (status == "Valid") { // Show loading indication
                    SystemMenuEditSubmitButton.setAttribute("data-kt-indicator", "on");
                    SystemMenuEditSubmitButton.disabled = true;
                    $.ajax({
                        url: globalURLCirculo + "system/updateMenu/",
                        type: "POST",
                        data: $('#system-form-menu-edit').serialize(),
                        success: function (data) {
                            if (data == "success") {
                                swal.fire("¡Registro!", "Excelente, el menú " + document.getElementById('vNameMenu').value + " se ha modificado, correctamente.", "success").then(function () {
                                    //location.reload();
